...
\begin{document}
	...
	\begin{definition}
		\leavevmode
		\begin{itemize}
			\item A \textit{character} on $A$ is a (\todo{non-zero? Continuous?}) algebra homomorphism $\chi:A\to\C$.
	...
	\begin{fact}\label{Fact: Maximal ideals in commutative Banach algebra}
		An ideal $m\subseteq A$ is maximal if and only if it is closed and has codimension 1.
	\end{fact}
...
\begin{proof}
	...
	For injectivity, let $\chi,\chi'$ be characters with $\ker \chi = \ker \chi'\eqcolon m$. Note that $A/m\cong \C$ by the argument above and fix such an isomorphism. Especially, both $\chi$ and $\chi'$ factor as a composition $A\to A/m\cong \C \to \C$, where the first map is the quotient map and the second map is the fixed isomorphism. But now, as any homomorphism of $\C$-algebras from $\C$ to $\C$ is the identity, the composition above is unique and $\chi=\chi'$.
\end{proof}
...
\end{lemma}
\begin{proof}
	Let $\chi\in \hat{A}$. By the proof of Proposition \ref{Prop: Spectrum is maximal ideals}, $\chi$ is already isomorphic to the quotient map $p:A\to A/(\ker\chi)$. But that quotient map satisfies $\norm{p(a)}=\norm{a+\ker\chi}\leq \norm{a}$ for all $a\in A$, hence so does $\chi$.
\end{proof}
\end{document}
